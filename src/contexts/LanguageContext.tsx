...
export const translations = {
  en: {
    ...
    validation: {
      ...
      warnings: 'tip(s)',
      // Best practice tips
      missingLabels: 'Adding labels helps with resource filtering and management',
      missingAppLabel: 'An "app" or "app.kubernetes.io/name" label helps tools identify your workload',
      missingLimits: 'Setting resource limits on "{name}" prevents resource contention in production',
      missingRequests: 'Setting resource requests on "{name}" helps the scheduler place pods optimally',
      missingProbes: 'Health probes on "{name}" enable automatic restart and traffic management',
      latestTag: 'Pinning a specific image tag on "{name}" ensures reproducible deployments',
      singleReplica: 'For production, consider replicas >= 2 to improve availability',
      loadBalancerCost: 'LoadBalancer type provisions cloud infrastructure and may incur costs',
      missingIngressClass: 'Specifying ingressClassName clarifies which controller processes these rules',
  ...
  zh: {
    ...
    validation: {
      ...
      warnings: '条建议',
      // Best practice tips
      missingLabels: '添加 labels 有助于资源的筛选和管理',
      missingAppLabel: '添加 "app" 或 "app.kubernetes.io/name" 标签便于工具识别工作负载',
      missingLimits: '为容器 "{name}" 设置资源上限可防止生产环境资源争用',
      missingRequests: '为容器 "{name}" 设置资源请求有助于调度器优化 Pod 放置',
      missingProbes: '为 "{name}" 配置健康检查探针可实现自动重启和流量管理',
      latestTag: '为 "{name}" 固定具体镜像版本可确保部署的可重复性',
      singleReplica: '生产环境建议设置 replicas >= 2 以提高可用性',
      loadBalancerCost: 'LoadBalancer 类型会创建云端负载均衡器，可能产生费用',
      missingIngressClass: '指定 ingressClassName 可明确使用的 Ingress 控制器',
